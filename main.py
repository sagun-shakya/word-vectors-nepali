...
# Local Modules.
from model.word2vec import PreTrained_Embedding
...
def train(args):
    ...
    w2v = PreTrained_Embedding(  mode = args.mode, 
                                 sg = args.sg,
    ...
    if args.save_model:
        w2v.save_model(args.model_dir, model, to_append=args.to_append)
...
def main():
    ...
    parser.add_argument('--txt_file', action = 'store_true', required = True, 
                        help = 'Is the input corpus a txt file?')
    parser.add_argument('file_path', type = str,
                        help = 'Path to the corpus file.')
    parser.add_argument('--save_model', action = 'store_true', 
                        help = 'Whether to store the model file.')
    parser.add_argument('--save_keyedvectors', action = 'store_true', 
                        help = 'Whether to store the keyed vectors.')
    parser.add_argument('--verbose', '-v', action = 'store_true', 
                        help = 'Whether to Show the hyperparameters used.')
    ...
    parser.add_argument('--to_append', type = str, default = None, help = "String to appendto saved model filename.")
    
    parser.add_argument('--mode', type = str, default = 'fasttext', 
                        help = 'Whether to use FastText or Word2Vec.')    
    parser.add_argument('--sg', type = int, default = 0, 
                        help = 'Whether to use skipgram (or C-BOW).')
    ...
    parser.add_argument('--loss', type = int, default = 0, 
                        help = 'Whether to use heirarchical loss.')
    parser.add_argument('--min_count', type = int, default = 3, 
                        help = 'Minimum frequency of a token to qualify for training.')
    ...
    parser.add_argument('--negative', type = int, default = 10, 
                        help = 'How many “noise words” should be drawn (usually between 5-20). If set to 0, no negative sampling is used.')
    parser.add_argument('--workers', type = int, default = int(cores) - 1, 
                        help = 'Number of CPU cores.')
    parser.add_argument('-e', '--num_iterations', type = int, default = 5,
